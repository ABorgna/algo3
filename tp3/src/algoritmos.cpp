...
pair<double, uint64_t> greedy_omNomNom(vector<int64_t> &orden) {
  ...
  while (!gyms.empty()){
    if (gyms.begin()->first <= potas_actual){
      ...
      double dist_candidato = graph.distance(nodo_actual, gym_candidato->second);

      for(auto it = gyms.begin(); it != gyms.end() and it->first <= potas_actual; it++) {
        if(graph.distance(nodo_actual, it->second) < dist_candidato) {
          gym_candidato = it;
          dist_candidato = graph.distance(nodo_actual, gym_candidato->second);
        }
      }
      ...
      if(not paradas.empty()){
        ...
        double dist_candidato = graph.distance(nodo_actual, *parada_mas_cercana);

        for(auto it = paradas.begin(); it != paradas.end(); it++) {
          if(graph.distance(nodo_actual, *parada_mas_cercana) < dist_candidato) {
            parada_mas_cercana = it;
            dist_candidato = graph.distance(nodo_actual, *parada_mas_cercana);
          }
        }
